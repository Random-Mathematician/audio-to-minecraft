...
def action(key, act):
	...
	if key in Input.KEYBOARD_KEYS:
		...
		else: raise ValueError("Invalid keyboard action: "+repr(act))
	elif key[:5] == "mouse":
		...
		else: raise ValueError("Invalid move action: "+repr(act))
	elif key[:5] == "click":
		...
		else: raise ValueError("Invalid click action: "+repr(act))
	elif key[:6] == "scroll":
		...
		else: raise ValueError("Invalid scroll action: "+repr(act))
...
def continuous_input():
	...
	global MOUSESCROLL
	while True: 
		hz = MOUSEMOVE["r"] - MOUSEMOVE["l"]
		vt = MOUSEMOVE["d"] - MOUSEMOVE["u"]
		sc = MOUSESCROLL["u"] - MOUSESCROLL["d"]
		if hz or vt: Input.move(hz, vt)
		if sc: Input.scroll(sc)
		sleep(0.01) # IMPORTANT: makes the system not overprocess
...
try:
	...
	while True:
		...
		lastnum = num
		num = pitchToNum(freq)
		# Show the current recieved pitch as a name
		print(numToName(num))
